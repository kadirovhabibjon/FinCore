...
def migrated_database(postgres_url: str, monkeypatch: pytest.MonkeyPatch):
    ...
    (engine + session factory) at it, so repositories/services/API calls
    under test hit a real, migrated database instead of the process's
    configured one.

    Function-scoped: each test gets a clean (migrated-then-downgraded)
    database without paying the cost of a new container per test.
    """
