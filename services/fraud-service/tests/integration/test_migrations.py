...
def test_upgrade_from_empty_database_creates_tables(
    ...
    assert "fraud_checks" in _table_names(postgres_url)


def test_downgrade_then_upgrade_is_repeatable(alembic_config: Config, postgres_url: str) -> None:
    """Same lesson as every other service: this migration's downgrade()
    explicitly drops the native enum type it creates, so a second
